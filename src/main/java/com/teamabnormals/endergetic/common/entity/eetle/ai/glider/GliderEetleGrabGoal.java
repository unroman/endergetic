...
public class GliderEetleGrabGoal extends Goal {
	...
	public boolean canUse() {
		...
		if (target == null || !target.isAlive() || GliderEetle.isEntityLarge(target) || ((IDataManager) target).getValue(EEDataProcessors.CATCHING_COOLDOWN) > 0 || target.getVehicle() instanceof GliderEetle || !this.glider.getPassengers().isEmpty()) {
			return false;
		}
