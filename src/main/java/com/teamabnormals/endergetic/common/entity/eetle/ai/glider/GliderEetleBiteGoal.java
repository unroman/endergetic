...
public class GliderEetleBiteGoal extends EndimatedGoal<GliderEetle> {
	...
	public GliderEetleBiteGoal(GliderEetle entity) {
	...
	public boolean canUse() {
		...
		if (target != null && target.isAlive() && (GliderEetle.isEntityLarge(target) || ((IDataManager) target).getValue(EEDataProcessors.CATCHING_COOLDOWN) > 0) && !(target.getVehicle() instanceof GliderEetle) && this.entity.getPassengers().isEmpty()) {
	...
	@Override
	public void start() {
		GliderEetle glider = this.entity;
		if (!glider.isFlying()) {
	...
	public boolean canContinueToUse() {
		if (this.entity.isGrounded()) return false;
		LivingEntity target = this.entity.getTarget();
		return target != null && target.isAlive() && (GliderEetle.isEntityLarge(target) || ((IDataManager) target).getValue(EEDataProcessors.CATCHING_COOLDOWN) > 0) && !(target.getVehicle() instanceof GliderEetle) && this.entity.getPassengers().isEmpty() && !this.entity.getNavigation().isDone();
	}
