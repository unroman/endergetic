...
public class BolloomBalloonItem extends Item {
		...
		if (block instanceof FenceBlock) {
			...
			for (Entity entity : world.getEntitiesOfClass(Entity.class, new AABB(pos))) {
				if (entity instanceof BolloomKnot) {
					if (((BolloomKnot) entity).hasMaxBalloons()) {
						return InteractionResult.FAIL;
					}
				}
			}
	...
	public static boolean canAttachBalloonToTarget(Entity target) {
		return !target.getType().is(EEEntityTypeTags.NOT_BALLOON_ATTACHABLE) && ((BalloonHolder) target).getBalloons().size() < (target instanceof Boat ? 4 : 6);
	}
	...
	public static class BalloonDispenseBehavior extends DefaultDispenseItemBehavior {
		...
		protected ItemStack execute(BlockSource source, ItemStack stack) {
			...
			if (state.canBeReplaced()) {
				...
				if (setKnot == null) {
					BolloomKnot.createStartingKnot(world, blockpos, ((BolloomBalloonItem) stack.getItem()).getBalloonColor());
					stack.shrink(1);
					return stack;
				} else {
